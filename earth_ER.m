...
folderpath = 'D:\Research_implementations\Paper_12\Earth_1\output';
...
fx_estimate= real(ifft2(ifftshift(F_estimate)));
Es(i)=(sum(sum(((fx_estimate(fx_estimate<0)).^2)*(d1)*(d1))))/(sum(sum((fx_estimate.^2)*(d1)*(d1))));
dd=fx_estimate>=0;
fx_estimate=(fx_estimate.*dd);
end
